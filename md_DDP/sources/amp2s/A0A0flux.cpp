...
double DT::A0A0H1H1fl(const double &cos_t, const double &s){
	using namespace PAR;
	return flux(s, MA0,MA0,MH1,MH1)*A0A0H1H1(cos_t, s);
}
...
double DT::A0A0H2H2fl(const double &cos_t, const double &s){
	using namespace PAR;
	return flux(s, MA0,MA0,MH2,MH2)*A0A0H2H2(cos_t, s);
}
double DT::A0A0ZZfl(const double &cos_t, const double &s){
	using namespace PAR;
	return flux(s, MA0,MA0,MZ,MZ)*A0A0ZZ(cos_t, s);
}
...
double DT::A0A0GGfl(const double &cos_t, const double &s){
	using namespace PAR;
	return flux(s, MA0,MA0,0,0)*A0A0GG(cos_t, s);
}
