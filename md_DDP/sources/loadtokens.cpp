...
namespace DT{
	void ModelInfo::load_tokens(){
		...
		token50 = (2*pow(m11,2) - 2*pow(MA0,2) + lam7*pow(vs,2))*cos(aDM) - lam7*v*vs*sin(aDM);
		token51 = CW;
		token52 = SW;
		token53 = pow(CW,3);
		token54 = pow(SW,3);
		token55 = 2*(m11 - MA0)*(m11 + MA0)*sin(aDM) + lam7*vs*(v*cos(aDM) + vs*sin(aDM));
		token56 = 1/SW;
		token57 = 1/v;
		token58 = 1/CW;
		token59 = MH3 - MHch;
		token60 = MH3 + MHch;
		token61 = -0.5;
		token62 = (2*pow(m11,2) - 2*pow(MHch,2) + lam7*pow(vs,2))*cos(aDM) - lam7*v*vs*sin(aDM);
		token63 = 2*(m11 - MHch)*(m11 + MHch)*sin(aDM) + lam7*vs*(v*cos(aDM) + vs*sin(aDM));
		...
		token76 = CKM3x2;
		token77 = CKM3x3;
		token78 = ((2*pow(m11,2) - 2*pow(MA0,2) + lam7*pow(vs,2))*pow(cos(aDM),2))/pow(v,2) - lam7*pow(sin(aDM),2);
		token79 = pow((2*pow(m11,2) - 2*pow(MA0,2) + lam7*pow(vs,2))*cos(aDM) - lam7*v*vs*sin(aDM),2);
		token80 = 2*pow(m11,2) - 2*pow(MA0,2) + lam7*(pow(v,2) + pow(vs,2));
		token81 = -(lam7*pow(cos(aDM),2)) + ((2*pow(m11,2) - 2*pow(MA0,2) + lam7*pow(vs,2))*pow(sin(aDM),2))/pow(v,2);
		token82 = pow(2*(m11 - MA0)*(m11 + MA0)*sin(aDM) + lam7*vs*(v*cos(aDM) + vs*sin(aDM)),2);
		token83 = -pow(MA0,2) + pow(MHch,2);
		token84 = MA0 - MHch;
		token85 = MA0 + MHch;
		token86 = sqrt(2);
		token87 = ((2*pow(m11,2) - 2*pow(MHch,2) + lam7*pow(vs,2))*pow(cos(aDM),2))/pow(v,2) - lam7*pow(sin(aDM),2);
		token88 = pow((2*pow(m11,2) - 2*pow(MHch,2) + lam7*pow(vs,2))*cos(aDM) - lam7*v*vs*sin(aDM),2);
		token89 = 2*pow(m11,2) - 2*pow(MHch,2) + lam7*(pow(v,2) + pow(vs,2));
		token90 = -(lam7*pow(cos(aDM),2)) + ((2*pow(m11,2) - 2*pow(MHch,2) + lam7*pow(vs,2))*pow(sin(aDM),2))/pow(v,2);
		token91 = pow(2*(m11 - MHch)*(m11 + MHch)*sin(aDM) + lam7*vs*(v*cos(aDM) + vs*sin(aDM)),2);
