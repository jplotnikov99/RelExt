...
double DT::lambda_t_e(){	
	...
	for(int a=0; a<3; a++){
		res_higgs -= 1./(2*mh[a]*mh[a]) * (Mu[gen]/vu) * Sh[a][1]* std::real(C_HL[a]);
	}
	...
	complex<double>cth = ZDT[0][0];
	complex<double>sth = ZDT[0][1];
	...
	for(int a=0; a<2;a++){
		res_squark -= (1./(4.*(msu2[a]-mn2))*std::real(CR_u[a]*std::conj(CL_u[a])));
	}
