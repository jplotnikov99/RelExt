...
double DT::A0pQVMu(const double &cos_t, const double &s){
	...
	return (2*(t*u - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ) + (s + u)* (MU*MU) -  (MU*MU*MU*MU))* (token21*token21)* (token33*token33)* (token6*token6))/(3.* (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
double DT::A0pQVMc(const double &cos_t, const double &s){
	...
	return (2*(t*u + (s + u)* (MC*MC) -  (MC*MC*MC*MC) - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ))* (token21*token21)* (token36*token36)* (token6*token6))/(3.* (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
double DT::A0pQVMt(const double &cos_t, const double &s){
	...
	return (2*(t*u - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ) + (s + u)* (MT*MT) -  (MT*MT*MT*MT))* (token21*token21)* (token39*token39)* (token6*token6))/(3.* (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
double DT::A0pQMUs(const double &cos_t, const double &s){
	...
	return ((t*u - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ) +  (MMU*MMU)*(-t + 2* (MphiQ*MphiQ) - 2* (MS*MS)) + s* (MS*MS) + u* (MS*MS) -  (MS*MS*MS*MS))* (token21*token21)* (token22*token22)* (token6*token6))/ (( (MchiM*MchiM) - t)*( (MchiM*MchiM) - t));
}
double DT::A0pQMUb(const double &cos_t, const double &s){
	...
	return ((t*u -  (MB*MB*MB*MB) +  (MB*MB)*(s + u - 2* (MMU*MMU)) - s* (MphiQ*MphiQ) - t* (MphiQ*MphiQ) - u* (MphiQ*MphiQ) +  (MMU*MMU)*(-t + 2* (MphiQ*MphiQ)) +  (MphiQ*MphiQ*MphiQ*MphiQ))* (token21*token21)* (token26*token26)* (token6*token6))/ (( (MchiM*MchiM) - t)*( (MchiM*MchiM) - t));
}
