...
double DT::A0pQVMu(const double &cos_t, const double &s){
	...
	return ((3*(t*u - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ) + (s + u)* (MU*MU) -  (MU*MU*MU*MU))* (token21*token21)* (token33*token33)* (token6*token6))/ (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
double DT::A0pQVMc(const double &cos_t, const double &s){
	...
	return ((3*(t*u + (s + u)* (MC*MC) -  (MC*MC*MC*MC) - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ))* (token21*token21)* (token36*token36)* (token6*token6))/ (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
double DT::A0pQVMt(const double &cos_t, const double &s){
	...
	return ((3*(t*u - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ) + (s + u)* (MT*MT) -  (MT*MT*MT*MT))* (token21*token21)* (token39*token39)* (token6*token6))/ (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
