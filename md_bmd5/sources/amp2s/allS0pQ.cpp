...
double DT::S0pQVMu(const double &cos_t, const double &s){
	...
	return ((48*(t*u - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ) + (s + u)* (MU*MU) -  (MU*MU*MU*MU))* (token21*token21)* (token3*token3)* (token33*token33)* (token6*token6))/ (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
double DT::S0pQVMc(const double &cos_t, const double &s){
	...
	return ((48*(t*u + (s + u)* (MC*MC) -  (MC*MC*MC*MC) - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ))* (token21*token21)* (token3*token3)* (token36*token36)* (token6*token6))/ (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
double DT::S0pQVMt(const double &cos_t, const double &s){
	...
	return ((48*(t*u - (s + t + u)* (MphiQ*MphiQ) +  (MphiQ*MphiQ*MphiQ*MphiQ) + (s + u)* (MT*MT) -  (MT*MT*MT*MT))* (token21*token21)* (token3*token3)* (token39*token39)* (token6*token6))/ (( (Mchi0*Mchi0) - t)*( (Mchi0*Mchi0) - t)));
}
double DT::S0pQMUs(const double &cos_t, const double &s){
	...
	return ((-48*(-(t*u) + (s + t + u)* (MphiQ*MphiQ) -  (MphiQ*MphiQ*MphiQ*MphiQ) - s* (MS*MS) - u* (MS*MS) +  (MMU*MMU)*(t - 2* (MphiQ*MphiQ) + 2* (MS*MS)) +  (MS*MS*MS*MS))* (token1*token1)* (token21*token21)* (token22*token22)* (token6*token6))/ (( (MchiM*MchiM) - t)*( (MchiM*MchiM) - t)));
}
double DT::S0pQMUb(const double &cos_t, const double &s){
	...
	return ((-48*(-(t*u) +  (MB*MB*MB*MB) +  (MB*MB)*(-s - u + 2* (MMU*MMU)) +  (MMU*MMU)*(t - 2* (MphiQ*MphiQ)) + s* (MphiQ*MphiQ) + t* (MphiQ*MphiQ) + u* (MphiQ*MphiQ) -  (MphiQ*MphiQ*MphiQ*MphiQ))* (token1*token1)* (token21*token21)* (token26*token26)* (token6*token6))/ (( (MchiM*MchiM) - t)*( (MchiM*MchiM) - t)));
}
