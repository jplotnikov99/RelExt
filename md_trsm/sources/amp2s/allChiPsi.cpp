...
double DT::ChiPsiGG(const double &cos_t, const double &s){
	return (32* (s*s)* (token11*token11)* (token13*token13)* (token20*token20)* (token34*token34))/( (token16*token16)*( (MH*MH*MH*MH) +  (s*s) +  (MH*MH)*(-2*s +  (WH*WH))));
}
