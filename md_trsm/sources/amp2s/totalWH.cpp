...
double DT::wwH(const double QCDaS){
	...
	std::vector<double> coupling2( {0.5*36* (lam*lam)* (vev*vev), 0.5* (vev*vev)* ((lam12* (cos( alpha)*cos( alpha)) + lam123*cos( alpha)*sin(alpha) + lam13* (sin(alpha)*sin(alpha)))*(lam12* (cos( alpha)*cos( alpha)) + lam123*cos( alpha)*sin(alpha) + lam13* (sin(alpha)*sin(alpha)))), ( (vev*vev)* ((lam123*cos( 2*alpha) - lam12*sin(2*alpha) + lam13*sin(2*alpha))*(lam123*cos( 2*alpha) - lam12*sin(2*alpha) + lam13*sin(2*alpha))))/4., 0.5* (vev*vev)* ((lam13* (cos( alpha)*cos( alpha)) - lam123*cos( alpha)*sin(alpha) + lam12* (sin(alpha)*sin(alpha)))*(lam13* (cos( alpha)*cos( alpha)) - lam123*cos( alpha)*sin(alpha) + lam12* (sin(alpha)*sin(alpha)))), 0.5* (AH*AH), 0.5*( (EL*EL*EL*EL)* (vev*vev)* (( (cw*cw) +  (sw*sw))*( (cw*cw) +  (sw*sw))*( (cw*cw) +  (sw*sw))*( (cw*cw) +  (sw*sw))))/(4.* (cw*cw*cw*cw)* (sw*sw*sw*sw)), ( (EL*EL*EL*EL)* (vev*vev))/(4.* (sw*sw*sw*sw)), 0.5* (GH*GH),  (ye*ye)/2.,  (ym*ym)/2.,  (ytau*ytau)/2.,  (yup*yup)/2.,  (yc*yc)/2.,  (yt*yt)/2.,  (ydo*ydo)/2.,  (ys*ys)/2.,  (yb*yb)/2., 0.5* (yt*yt)/2. } );
