...
namespace DT
{
    ...
    double Mrun::RunM(const double q, const int NF0, const int iif)
    {
        ...
        if (NF == 3)
        {
            if (QQS < amc)
            {
                YMSB[3] = YMSB[2] * CQ(alphaS(AM[3]) / M_PI, 3) / CQ(alphaS(QQS) / M_PI, 3);
            }
            else
            {
                YMSB[3] = ams * CQ(alphaS(AM[3]) / M_PI, 4) / CQ(alphaS(QQS) / M_PI, 4);
                YMSB[2] = YMSB[3];
            }
            YMSB[4] = YMSB[3] * CQ(alphaS(AM[4]) / M_PI, 4) / CQ(alphaS(AM[3]) / M_PI, 4);
            YMSB[5] = YMSB[4] * CQ(alphaS(AM[5]) / M_PI, 5) / CQ(alphaS(AM[4]) / M_PI, 5);
        }
        else if (NF == 4)
        {
            YMSB[3] = amcb * CQ(alphaS(AM[3]) / M_PI, 4) / CQ(alphaS(QQC) / M_PI, 4);
            YMSB[2] = YMSB[3] * CQ(alphaS(QQS) / M_PI, 3) / CQ(alphaS(AM[3]) / M_PI, 3);
            YMSB[4] = YMSB[3] * CQ(alphaS(AM[4]) / M_PI, 4) / CQ(alphaS(AM[3]) / M_PI, 4);
            YMSB[5] = YMSB[4] * CQ(alphaS(AM[5]) / M_PI, 5) / CQ(alphaS(AM[4]) / M_PI, 5);
        }
        else if (NF == 5)
        {
            YMSB[4] = XMSB;
            YMSB[3] = YMSB[4] * CQ(alphaS(AM[3]) / M_PI, 4) / CQ(alphaS(AM[4]) / M_PI, 4);
            YMSB[2] = YMSB[3] * CQ(alphaS(QQS) / M_PI, 3) / CQ(alphaS(AM[3]) / M_PI, 3);
            YMSB[5] = YMSB[4] * CQ(alphaS(AM[5]) / M_PI, 5) / CQ(alphaS(AM[4]) / M_PI, 5);
        }
        else if (NF == 6)
        {
            YMSB[5] = XMSB;
            YMSB[4] = YMSB[5] * CQ(alphaS(AM[4]) / M_PI, 5) / CQ(alphaS(AM[5]) / M_PI, 5);
            YMSB[3] = YMSB[4] * CQ(alphaS(AM[3]) / M_PI, 4) / CQ(alphaS(AM[4]) / M_PI, 4);
            YMSB[2] = YMSB[3] * CQ(alphaS(QQS) / M_PI, 3) / CQ(alphaS(AM[3]) / M_PI, 3);
        }
