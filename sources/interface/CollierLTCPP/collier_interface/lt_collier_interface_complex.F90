...
!     * Replace parameter pi by pi2_6 = pi2_6, which is used everywhere
...
#include "lt_collier.h"
  ...
  RealType, parameter :: pi2_6 = pi**2/6d0
...
subroutine clearcache
  use COLLIER
  implicit none
  call InitEvent_cll(1) ! only one cache system as in LoopTools
...
subroutine setparam_collier(uvdiv,delta_uv,mu2,lambda)
  use COLLIER
  implicit none
...
subroutine init_collier(Nmax,ncache)
  ...
  implicit none
#include "lt_ff.h"
  ! ncache = number of cache systems to be used (one for each channel/initial state)
  ! Nmax   = maximal N up to which N-point integrals are cached
  ! Rmax   = maximal rank of loop integrals (usually = Nmax) ! CB: careful, due to rank = Nmax+1 being used above, this should also be set to Rmax = Nmax + 1, and not Rmax = Nmax
  integer, intent(in) :: Nmax,ncache
  integer :: Rmax
!  if (Nmax.eq.Rmax) then
!    write (*,*) 'init_collier: ABORTING. Nmax = Rmax can lead to problems, set Rmax = Nmax + 1!'
!    STOP
!  end if
  Rmax = Nmax+1
