...
int &VecInt::operator[](const size_t i) { return v[i]; }

const int &VecInt::operator[](const size_t i) const { return v[i]; }
...
VecDoub::VecDoub(const size_t n, const double &a)
    : nn(n), v(n > 0 ? new double[n] : nullptr) {
    for (size_t i = 0; i < nn; i++) v[i] = a;
}

VecDoub::VecDoub(const VecDoub &a)
    : nn(a.nn), v(nn > 0 ? new double[nn] : nullptr) {
    for (size_t i = 0; i < nn; i++) v[i] = a[i];
}
...
double &VecDoub::operator[](const size_t i) { return v[i]; }

const double &VecDoub::operator[](const size_t i) const { return v[i]; }
...
MatDoub::MatDoub(const size_t n, const size_t m)
    : nrows(n), ncols(m), v(n > 0 ? new double *[n] : nullptr) {
    size_t i, nel = m * n;
...
MatDoub::MatDoub(const MatDoub &a)
    ...
    if (v) v[0] = nel > 0 ? new double[nel] : nullptr;
    for (i = 1; i < nrows; i++) v[i] = v[i - 1] + ncols;
    for (i = 0; i < nrows; i++)
        for (j = 0; j < ncols; j++) v[i][j] = a[i][j];
}
...
size_t MatDoub::rows() { return nrows; }

size_t MatDoub::cols() { return ncols; }
...
double *MatDoub::operator[](const size_t i) { return v[i]; }

const double *MatDoub::operator[](const size_t i) const { return v[i]; }
...
double **Mat3DDoub::operator[](const size_t i) { return v[i]; }

const double *const *Mat3DDoub::operator[](const size_t i) const {
    return v[i];
}

size_t Mat3DDoub::dim1() { return nn; }
size_t Mat3DDoub::dim2() { return mm; }
size_t Mat3DDoub::dim3() { return kk; }
