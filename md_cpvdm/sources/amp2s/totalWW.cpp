...
double DT::wWveE(){
	 if(heaviDecays(MW,Mnue,Me)){
      ...
      (Mnue*Mnue))* (MW*MW) - 2* (MW*MW*MW*MW) +  (( (Me*Me) - \
 
      (Mnue*Mnue))*( (Me*Me) -  (Mnue*Mnue)))))/(Pi* (MW*MW*MW*MW*MW)*\
...
double DT::wWvmM(){
	 if(heaviDecays(MW,Mnum,MM)){
      ...
      (Mnum*Mnum))* (MW*MW) - 2* (MW*MW*MW*MW) +  (( (MM*MM) - \
 
      (Mnum*Mnum))*( (MM*MM) -  (Mnum*Mnum)))))/(Pi* (MW*MW*MW*MW*MW)*\
...
double DT::wWvtTA(){
	 if(heaviDecays(MW,Mnut,MTA)){
      ...
      +  (MTA*MTA))* (MW*MW) - 2* (MW*MW*MW*MW) +  (( (Mnut*Mnut) - \
...
double DT::wwW(){
	 ...
	 return ( wWH1Hc() + wWH2Hc() + wWH3Hc() + wWhW() + wWAW() + wWZW() +\
      ...
      wWveE() + wWvmM() + wWvtTA() + wWuD() + wWuS() + wWuB() + wWcD() +\
 
      wWcS() + wWcB() + wWtD() + wWtS() + wWtB() );
}
