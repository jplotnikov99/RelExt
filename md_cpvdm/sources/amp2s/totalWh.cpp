...
double DT::whEe(){
	 if(heaviDecays(mHsm,Me,Me)){
		 return ( (gc63*gc63)*(-4* (Me*Me) +  (mHsm*mHsm))*sqrt(-4* (Me*Me)*\
 
   (mHsm*mHsm) +  (mHsm*mHsm*mHsm*mHsm)))/(8.*Pi* (mHsm*mHsm*mHsm));
	 }
	 else{ return 0; }
...
double DT::whMm(){
	 if(heaviDecays(mHsm,MM,MM)){
		 return ( (gc65*gc65)*( (mHsm*mHsm) - 4* (MM*MM))*sqrt(\
 
   (mHsm*mHsm*mHsm*mHsm) - 4* (mHsm*mHsm)* (MM*MM)))/(8.*Pi* (mHsm*mHsm*mHsm))
...
double DT::whTAta(){
	 if(heaviDecays(mHsm,MTA,MTA)){
		 return ( (gc67*gc67)*( (mHsm*mHsm) - 4* (MTA*MTA))*sqrt(\
...
double DT::whUu(){
	 if(heaviDecays(mHsm,MU,MU)){
		 return (3* (gc85*gc85)*( (mHsm*mHsm) - 4* (MU*MU))*sqrt(\
 
   (mHsm*mHsm*mHsm*mHsm) - 4* (mHsm*mHsm)* (MU*MU)))/(8.*Pi* (mHsm*mHsm*mHsm))
...
double DT::whCc(){
	 if(heaviDecays(mHsm,MC,MC)){
		 return (3* (gc81*gc81)*(-4* (MC*MC) +  (mHsm*mHsm))*sqrt(-4* (MC*MC)*\
...
double DT::whTt(){
	 if(heaviDecays(mHsm,MT,MT)){
		 return (3* (gc83*gc83)*( (mHsm*mHsm) - 4* (MT*MT))*sqrt(\
 
   (mHsm*mHsm*mHsm*mHsm) - 4* (mHsm*mHsm)* (MT*MT)))/(8.*Pi* (mHsm*mHsm*mHsm))
...
double DT::whDd(){
	 if(heaviDecays(mHsm,MD,MD)){
		 return (3* (gc50*gc50)*(-4* (MD*MD) +  (mHsm*mHsm))*sqrt(-4* (MD*MD)*\
...
double DT::whSs(){
	 if(heaviDecays(mHsm,MS,MS)){
		 return (3* (gc52*gc52)*( (mHsm*mHsm) - 4* (MS*MS))*sqrt(\
 
   (mHsm*mHsm*mHsm*mHsm) - 4* (mHsm*mHsm)* (MS*MS)))/(8.*Pi* (mHsm*mHsm*mHsm))
...
double DT::whBb(){
	 if(heaviDecays(mHsm,MB,MB)){
		 return (3* (gc48*gc48)*(-4* (MB*MB) +  (mHsm*mHsm))*sqrt(-4* (MB*MB)*\
